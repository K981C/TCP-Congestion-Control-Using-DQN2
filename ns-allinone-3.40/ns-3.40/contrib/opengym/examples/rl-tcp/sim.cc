...
int main (int argc, char *argv[])
{
  ...
  // NEW: Flag to enable NetAnim XML generation
  ...
  // NEW: NetAnim option
  cmd.AddValue ("enableNetAnim", "Generate NetAnim XML trace file", enableNetAnim);
  cmd.Parse (argc, argv);
  ...
  NS_LOG_UNCOND("--enableNetAnim: " << enableNetAnim); // NEW: log the flag
  ...
  // ========== NEW: NetAnim XML generation ==========
  if (enableNetAnim)
  {
    // Build the XML filename from prefix
    std::string animFile = prefix_file_name + "-netanim.xml";
    // Create AnimationInterface object
    AnimationInterface anim(animFile);
    // Optional: set mobility polling interval (default is 1s)
    anim.SetMobilityPollInterval(Seconds(1));

    // Set positions for better visualization of the dumbbell topology
    // d.GetLeft() returns the left router and d.GetLeft(i) returns the i'th left leaf node
    // d.GetRight() returns the right router and d.GetRight(i) returns the i'th right leaf node
    // Positions: left router at (0,0), right router at (10,0)
    // Left leaves at x=-5, y = i*2 - (nLeaf-1) ; right leaves at x=15, y similarly
    Ptr<Node> leftRouter = d.GetLeft();
    Ptr<Node> rightRouter = d.GetRight();
    anim.SetConstantPosition(leftRouter, 0.0, 0.0);
    anim.SetConstantPosition(rightRouter, 10.0, 0.0);

    // Place leaf nodes
    double yOffset = 2.0; // spacing between leaves
    for (uint32_t i = 0; i < nLeaf; ++i)
    {
      Ptr<Node> leftLeaf = d.GetLeft(i);
      Ptr<Node> rightLeaf = d.GetRight(i);
      // Center the leaves around y=0
      double y = (i - (nLeaf - 1) / 2.0) * yOffset;
      anim.SetConstantPosition(leftLeaf, -5.0, y);
      anim.SetConstantPosition(rightLeaf, 15.0, y);
    }
    // Note: The AnimationInterface will write the XML file when it goes out of scope (after Simulator::Run)
    // We keep it alive until the end of main.
  }
  // ================================================
