...
def find_ns3_path(cwd=None):
    # Directory containing this Python file
    this_dir = os.path.dirname(os.path.abspath(__file__))

    # Go up until project root
    project_root = os.path.abspath(
        os.path.join(this_dir, "../../../../../")
    )
    
    print("project_root=", project_root)
    # Construct ns3 executable path
    ns3_path = os.path.join(
        project_root,
        "simulation",
        "ns-allinone-3.40",
        "ns-3.40",
    ...
    if not os.path.isfile(ns3_path):
        print("ns3 executable not found:")
...
def start_sim_script(port=5555, sim_seed=0, sim_args={}, debug=False):
    ...
    cwd = os.getcwd()
    cwd = os.path.join(cwd,"../simulation/ns-allinone-3.40/ns-3.40/contrib/opengym/examples/rl-tcp")
    sim_script_name = os.path.basename(cwd)
    ns3_path = find_ns3_path(cwd)
    base_ns3_dir = os.path.dirname(ns3_path)
